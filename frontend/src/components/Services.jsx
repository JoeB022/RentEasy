...
const Services = () => {
  ...
  const handleDelete = (id) => {
    ...
    toast.success('Service removed from history.');
  };
        ...
        {services.map((service) => (
          ...
          >
            ...
            <button
              ...
              className="mt-4 w-full bg-[#003B4C] text-white py-2 rounded-md hover:bg-blue-700 transition"
            >
              Book Now
            </button>
          </div>
                ...
                {bookingHistory.map((entry) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-medium">
                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-semibold transition duration-300 ease-in-out ${
                          entry.status === 'Booked'
                      ...
                      {entry.status === 'Booked' ? (
                        <>
                          <button
                            ...
                            className="text-green-600 hover:underline text-xs transition"
                          >
                            Mark as Completed
                          </button>
                          <button
                            onClick={() => handleReschedule(entry)}
                            className="text-blue-600 hover:underline text-xs transition"
                          >
                            Reschedule
                          </button>
                          <button
                            onClick={() => handleStatusChange(entry.id, 'Cancelled')}
                            className="text-red-600 hover:underline text-xs transition"
                          >
                            Cancel
                          </button>
                        </>
